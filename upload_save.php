...
if ($errflag) {
    ...
    session_write_close();
    echo("</script>location.href = 'uploadreport.php'</script>");
    exit();
}
    ...
    if (($ext != "exe") && ($_FILES["uploaded_file"]["type"] != "application/x-msdownload")) {
        ...
        if (!file_exists($newname)) {
           ...
           // if ((move_uploaded_file($_FILES['uploaded_file']['tmp_name'], $newname))) {
                ...
                $qry2 = "INSERT INTO files (fdesc,floc,fdatein,fname) VALUES ('$filedesc','$newname',NOW(),,'$name')";
                ...
                if ($result2) {
                    ...
                    if ($errflag) {
                        ...
                        <script type="text/javascript">
                          window.location="labreport.php<?php echo '?id='.$id_class; ?>";                          
                        </script>
                        <?php
                    ...
                    if ($errflag) {
                        $_SESSION['ERRMSG_ARR'] = $errmsg_arr;
                        session_write_close();
                        header("<script>location.href = 'labreport'</script>");
                        exit();
                    }
                }
            } else {
            ...
            if ($errflag) {
                $_SESSION['ERRMSG_ARR'] = $errmsg_arr;
                session_write_close();
                header("<script>location.href = 'labreport.php'</script>");
                exit();
            }
        }
    } else {
